...
"""
    ...
    def bug_bounty(self, target: str, validate: bool = True, force: bool = False,
        ...
        # UNKNOWN-scope targets used to fall through into the full pipeline (a multi-minute
        # nmap+nuclei+katana scan launched from a single chat command) — a real chat-hang +
        # safety bug surfaced by dogfood. Treat unknown like out: refuse fast, require force.
    ...
    def full_recon(self, target: str, force: bool = False, discover: bool = False) -> dict:
        ...
        if discover:
            ...
            sections["discovery"] = "Skipped (opt-in — pass discover=True to run ffuf/gobuster)."
...
"""
    ...
    # class -> (finding template, CWE class word, human name). Same OOB listener; only the injection differs.
    _OAST_KINDS = {
    ...
    # RUN
    # =====================================
    # ── F4 execution-timeline surface (JARVIS chat parity with the recon CLI) ──
    ...
    def run(
        ...
        try:
            ...
            if action == "nmap_scan":
            ...
            elif action == "cors_check":
                return self.cors_check(parameters.get("urls", "") or target)

            elif action == "secret_scan":
                return self.secret_scan(parameters.get("target", "") or target, parameters.get("urls"))
                ...
                eps = list(parameters.get("endpoints") or [])
                if not eps:
                    try:
                        from core import target_profiles as tp
                        host = (target or "").split("//")[-1].split("/")[0]
                        eps = list(tp._load().get(tp._norm(host), {}).get("endpoints", []))
                    except Exception:
                        eps = []
                    base = target if str(target).startswith("http") else "http://" + str(target)
                    if base not in eps:
                        eps.append(base)
                return self.auth_matrix(eps, parameters.get("owner", ""),
                                        parameters.get("attacker", ""), parameters.get("write", False))
                ...
                return self.timeline_show(parameters.get("run_id", "") or target)

            elif action == "make_package":
                return self.make_package(parameters.get("run_id", "") or target)

            elif action == "replay_run":
                return self.replay_run(parameters.get("run_id", "") or target,
                                       parameters.get("step", ""))
